...
class Kriah extends CI_Controller {
	...
	{
	  	if($this->session->logged_in && $this->session->user_type == 'superadmin'){
		...
		} elseif($this->session->logged_in && $this->session->user_type == 'admin'){
			$data['title'] = 'Dashboard';
			$data['left_sidebar']= $this->load->view('sections/left_sidebar_admin', '', true);
			$data['header']= $this->load->view('sections/header', '', true);
			$data['body']= $this->load->view('dashboard', '', true);
			$this->load->view('template', $data);
		} elseif($this->session->logged_in && $this->session->user_type == 'coach'){
		    $data['title'] = 'Dashboard';
			$data['left_sidebar']= $this->load->view('sections/left_sidebar_coach', '', true);
			$data['header']= $this->load->view('sections/header', '', true);
			$data['body']= $this->load->view('dashboard', '', true);
			$this->load->view('template', $data);
		} elseif($this->session->logged_in && $this->session->user_type == 'agency'){
			$data['title'] = 'Dashboard';
			$data['left_sidebar']= $this->load->view('sections/left_sidebar_agency', '', true);
			$data['header']= $this->load->view('sections/header', '', true);
			$data['body']= $this->load->view('dashboard', '', true);
			$this->load->view('template', $data);
		} else {
		...
		if($this->input->method() == 'post') {
			...
			if ($this->form_validation->run() == TRUE) { 
				if($this->Agency_model->update_agency($agency_id)) {	// update record			
					$this->session->set_flashdata('success', 'Agency updated successfuly');
					redirect(base_url('kriah/all_agencies'));
				} else {
					$this->session->set_flashdata('error', 'Agency not updated successfuly, try again');
					redirect(base_url('kriah/all_agencies'));	
				}
			} else {	
				// returnn with form validation errors
	...
	public function delete_agency()
	{
		if($this->input->method() == 'get') {
			...
			if($this->Agency_model->delete_agency($id)) {
				$this->session->set_flashdata('success', 'Agency deleted successfuly');
				echo json_encode(['status' => 'success']);
			} else {
				$this->session->set_flashdata('error', 'Agency not deleted successfuly, try again');
	...
	public function change_agency_user_status($status=null)
	{
		...
		if($this->Agency_model->changeAgencyUserStatus($user_id, $status)==true)
		{
		...
		if($this->input->method() == 'post') {
			...
			if ($this->form_validation->run() == TRUE) { 
				...
				if($this->Agency_model->insert_agency_user($agency_id)) {	// insert record			
					$this->session->set_flashdata('success', 'Agency User added successfuly');
					redirect('manage-agency-users/'.$agency_id);
					//$this->all_agency_users();
				} else {
					$this->session->set_flashdata('error', 'Agency User not added successfuly, try again');
					redirect('manage-agency-users/'.$agency_id);	
				}
			} else {	
				// returnn with form validation errors
	...
	public function edit_agency_user() 
	{
		...
		if($this->input->method() == 'post') {
			...
			$this->form_validation->set_rules('phone', 'Phone', 'trim|required|numeric');	
			if ($this->form_validation->run() == TRUE) { 
				if($this->Agency_model->update_agency_user()) {	// update record			
	...
	public function delete_agency_user()
	{
		if($this->input->method() == 'get') {
			...
			if($this->Agency_model->delete_agency_user($user_id)) {
				$this->session->set_flashdata('success', 'Agency User deleted successfuly');
				echo json_encode(['status' => 'success']);
			} else {
				$this->session->set_flashdata('error', 'Agency User not deleted successfuly, try again');
				echo json_encode(['status' => 'error']);
			}
		}
	}
